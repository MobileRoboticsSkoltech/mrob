...
// TODO integrate all these below
...
class FGraphDensePy : public FGraphSolveDense
{
public:
    /**
    ...
    id_t add_factro_1pose_point2plane(const py::EigenDRef<const Mat31> z_point, const py::EigenDRef<const Mat41> z_plane,
            id_t nodePoseId, const py::EigenDRef<const Mat1> obsInf)
    {
        auto n1 = this->get_node(nodePoseId);
        std::shared_ptr<mrob::Factor> f(new mrob::Factor1PosePoint2Plane(z_point,z_plane,n1,obsInf));
        this->add_factor(f);
        return f->get_id();
    }
